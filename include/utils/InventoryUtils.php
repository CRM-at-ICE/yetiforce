...
function isRecordExistInDB($fieldData, $moduleMeta, $user)
{
	...
	$log = vglobal('log');
	$moduleFields = $moduleMeta->getModuleFields();
	$moduleModel = Vtiger_Module_Model::getInstance($moduleMeta->getTabName());
	if($moduleModel->isInventory()){
			$inventory = Vtiger_InventoryField_Model::getInstance($moduleMeta->getTabName());
...
function importRecordNew($obj, $inventoryFieldData, $lineItemDetails)
{
	...
	$webserviceObject = VtigerWebserviceObject::fromName($adb, $moduleName);
	$inventoryOperation = new VtigerNewInventoryOperation($webserviceObject, $obj->user, $adb, $log);
